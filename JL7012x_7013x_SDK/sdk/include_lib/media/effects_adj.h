...
#define mSECTION_MAX (10)
#define MULTI_BAND_DRC 0
#else
...
struct tunning_eq_seg {
    u16 filt_type;       //滤波器类型EQ_IIR_TYPE
    u16 center_freq;           //中心截止频率(20~20KHz)
    float q;            //Q值(0.3~30)
    float min_gain;     //增益的最小值(-12~0)
    float max_gain;     //增益的最大值(0~12)
};
#define TUNNING_EQ_SETION  3
struct tunning_eq_tool {
    struct tunning_eq_seg seg[TUNNING_EQ_SETION];// 0:low  1:mid 2:high
};
...
struct eff_parm {
    ...
    struct tunning_eq_tool tunning_eq_parm;
};
#endif
